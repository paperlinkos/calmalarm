...
class _PodDashboardScreenState extends ConsumerState<PodDashboardScreen> {
  ...
  void _toggleSummary([bool? show]) {
    ...
    if (_isSummaryExpanded == target) return;
    setState(() {
    ...
    return Scaffold(
      ...
      body: GestureDetector(
        onVerticalDragUpdate: (details) {
          // Downward drag gesture anywhere near top triggers quick summary
          if (details.delta.dy > 6 && (_scrollController.hasClients && _scrollController.offset <= 20)) {
            _toggleSummary(true);
          } else if (details.delta.dy < -6 && _isSummaryExpanded) {
            _toggleSummary(false);
          }
        },
        child: NotificationListener<ScrollNotification>(
          onNotification: (ScrollNotification notification) {
            // Detect pull-down scroll at top of screen
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                // Pull-down Visual Hint / Toggle Button
                GestureDetector(
